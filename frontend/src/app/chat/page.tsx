...
export default function ChatMasterPage() {
  ...
  // Selected document viewer state for global chat context side-by-side view
  const [selectedDocId, setSelectedDocId] = useState<string | null>(null);
  const [selectedDocHighlight, setSelectedDocHighlight] = useState("");
  const [selectedDocFilename, setSelectedDocFilename] = useState("");
  const [selectedDocMarkdown, setSelectedDocMarkdown] = useState("");
  ...
  const filteredDocs = useMemo(() => {
    return documents
      .filter((doc) => {
        ...
        return doc.status === "processed" || doc.status === "vector_processed";
      })
      .filter((doc) =>
        doc.filename.toLowerCase().includes(searchQuery.toLowerCase())
      );
  }, [documents, searchQuery, canSeeAll]);

  const docNameMap = useMemo(() => {
    return new Map(documents.map((d) => [d.id, d.filename]));
  }, [documents]);

  const handleSessionClick = useCallback(async (session: ChatSession, e: React.MouseEvent) => {
  ...
  const handleGlobalSend = useCallback(async (q: string) => {
    if (!q.trim() || globalLoading) return;
  ...
  }, [globalLoading, globalSessionId]);

  const handleSourceClick = useCallback(async (src: { file: string; lines: string }, e: React.MouseEvent) => {
    e.preventDefault();
    const docId = src.file.replace(/\.md$/i, "");
    setSelectedDocHighlight(src.lines);
    
    if (selectedDocId === docId) {
      // Already loaded, just update highlight
      return;
    }
    ...
    try {
      ...
      const [detailRes, mdRes] = await Promise.all([
        fetch(`${API}/documents/${docId}`, { headers }).then(r => r.json()),
        fetch(`${API}/documents/${docId}/markdown`, { headers }).then(r => r.json())
      ]);
      
      setSelectedDocFilename(detailRes.filename || docId);
      setSelectedDocMarkdown(mdRes.markdown || "# Không thể tải tài liệu");
    } catch (err) {
      console.error(err);
      setSelectedDocMarkdown("# Không thể tải tài liệu");
    } finally {
      setSelectedDocLoading(false);
    }
  }, [selectedDocId]);

  const handleDocClick = useCallback(async (docId: string, e: React.MouseEvent) => {
    e.preventDefault();
    setSelectedDocHighlight("");
    
    if (selectedDocId === docId) {
      // Already loaded, just clear highlight
      return;
    }
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${(doc.status === "processed" || doc.status === "vector_processed") ? "bg-emerald-500" : "bg-amber-400 animate-pulse"}`} />
                        {(doc.status === "processed" || doc.status === "vector_processed") ? "Sẵn sàng" : "Đang xử lý"}
                      </span>
  ...
  }, [searchQuery, sessions, filteredDocs, docNameMap, handleSessionClick, handleDeleteSession]);

  const memoizedGlobalMessagesList = useMemo(() => {
    return (
  ...
  }, [globalMessages, globalLoading, docNameMap, handleSourceClick, handleDocClick]);
  ...
  return (
    <ProtectedRoute>
      <div className="flex h-full w-full overflow-hidden">
        {/* LEFT: Documents + Sessions OR Document Viewer */}
        {selectedDocId ? (
            ...
            <header className="h-14 flex items-center justify-between px-4 border-b border-theme-light bg-tertiary">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-theme bg-secondary text-xs font-bold text-muted hover:text-secondary hover:bg-tertiary hover:border-theme-accent cursor-pointer transition-all"
                >
                  <ChevronLeft className="w-4 h-4" />
                  Đóng tài liệu
                </button>
                <div className="flex items-center gap-1.5 text-xs font-semibold text-muted ml-2">
                  <FileText className="w-4 h-4 text-emerald-400 dark:text-indigo-400" />
                  <span className="truncate max-w-[240px]">{selectedDocFilename}</span>
                </div>
              </div>
              <div className="flex items-center gap-1.5">
                <button
        ...
        <div className="w-[400px] flex-shrink-0 flex flex-col glass-panel border-l border-theme-light">
          ...
          {/* Messages */}
          {memoizedGlobalMessagesList}

          {/* Chat Input */}
          <GlobalChatInput onSend={handleGlobalSend} loading={globalLoading} />
        </div>
      </div>
    </ProtectedRoute>
  );
...
function GlobalChatInput({ onSend, loading }: GlobalChatInputProps) {
